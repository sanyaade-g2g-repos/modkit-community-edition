...
public class Editor extends JFrame implements RunnerListener {
  ...
  public Editor(Base ibase, String path, int[] location) {
    ...
    upper.add(status, BorderLayout.NORTH);
...
//    System.out.println("t5");
  ...
  protected void setPlacement(int[] location) {
    setBounds(location[0], location[1], location[2], 90);
    if (location[4] != 0) {
...
//      code.undo = new UndoManager();
  ...
  protected boolean handleOpenInternal(String path) {
    ...
    setTitle("Arduino "+ Base.VERSION_NAME+" - Modkit Edition" );
...
//    }
 ...
 public void startModkitCopyPasteListener() {
    ...
    Thread t = new Thread(new Runnable() {
        public void run() {
          try { 
		      String modkitCode = new String();
			  String copyBuff = new String();
			  TextTransfer textTransfer = new TextTransfer();
             
		       while(true){
			     
    		       copyBuff = textTransfer.getClipboardContents();
				   if (copyBuff.contains("http://modk.it/arduino")){
    				   if(!modkitCode.equals(copyBuff)){
	    			       modkitCode=copyBuff;
		    		       textarea.selectAll();
			    	       textarea.paste();
						   handleExport();
